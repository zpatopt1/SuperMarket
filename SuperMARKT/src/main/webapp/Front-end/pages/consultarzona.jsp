         ...
         <table id="zonasTable" class="table">
		  ...
		  <tbody>
		    ...
		    <tr>
		      ...
		      <td><%= zona.getNome() %></td>
		      <td><%= zona.getLocal().getNome() %></td>
		      <td><%= zona.getLocal().getTipoLocal() %></td>
		
		      <!-- EDITAR -->
		      <td>
		        <button type="button" class="btn-guardar" style="background-color:gray;"
		          onclick="abrirModal(<%= zona.getIdZona() %>, '<%= zona.getNome() %>', <%= zona.getLocal().getIdLocal() %>)">
		          Editar
		        </button>
		      </td>
		      ...
		      <!-- APAGAR -->
		      <td>
		        <form method="post" action="${pageContext.request.contextPath}/ConsultarZonaServlet">
          ...
          <div class="registo-card">
			...
			<form action="${pageContext.request.contextPath}/ConsultarZonaServlet" method="POST">
			  ...
			  <div class="input-group full-width">
			    ...
			    <input type="number" id="modal_id_local" name="id_local" required />
			  </div>
        ...
        <div id="modalAddZona" class="modal">
		  <div class="registo-card">
		    ...
		    <form action="${pageContext.request.contextPath}/ConsultarZonaServlet" method="POST">
		      ...
		      <div class="input-group full-width">
		        <label>ID Local</label>
		        <input type="number" name="modal_add_id_local" required />
		      </div>
        ...
        <script>
          $(document).ready(function() {
            var table = $('#zonasTable').DataTable({
              ...
              language: {
                search: 'Pesquisar:',
                lengthMenu: 'Mostrar _MENU_ Zona por página',
                info: 'Mostrando _START_ a _END_ de _TOTAL_ Zona',
                paginate: { previous: '« Anterior', next: 'Próximo »' },
                zeroRecords: 'Nenhuma Zona encontrado'
              },
          ...
          function abrirModalAddZona() {
            document.getElementById('modalAddZona').style.display = 'flex';
          }

          function fecharModalAddZona() {
            document.getElementById('modalAddZona').style.display = 'none';
          }
        </script>
      </section>
    </main>
